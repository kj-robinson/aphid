...
# make a totaled column of aphids found on focal plant
countdata <- countdata %>%
...
#### APHID ABUNDANCE AND TEMPERATURE FIGURE (Figure 1) ####
...
# model (original - no AR1, guassian distribution)
lmfullmodel<-lmer(focal_aphids ~ predatortreatment * warmingtreatment * 
                    jd_sc + predatortreatment * warmingtreatment * 
                    I(jd_sc^2) + (1 | cage), data = countdata)
...
#### DAY BY DAY APHID ABUNDANCE ANALYSIS ####
...
Anova(lm198, type=2)
coef(summary(lm198))
#significant effect of predators and an interaction with warmingxpred

#July 21
lm202 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, 
                 family = nbinom2, data = jd202)
Anova(lm202, type=2)
coef(summary(lm202))
#nothing significant

#July 24
lm205 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, 
                 family = nbinom2, data = jd205)
Anova(lm205, type=2)
#predators significant
coef(summary(lm205))
...
# July 31
lm212 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, 
                 family = nbinom2, data = jd212)
Anova(lm212, type=2)
#nothing significant
coef(summary(lm212))

#August 5
lm217 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd217)
Anova(lm217, type=2)
#nothing significant
coef(summary(lm217))

#August 7
lm219 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2,  data = jd219)
Anova(lm219, type=2)
coef(summary(lm219))
#nothing significant

#August 11
lm223 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd223)
Anova(lm223, type=2)
#nothing significant
coef(summary(lm223))

#August 14
lm226 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd226)
Anova(lm226, type=2)
#nothing significant
coef(summary(lm226))

#August 18
lm230 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd230)
Anova(lm230, type=2)
coef(summary(lm230))
#strong significant effect of warmingtreatment

#August 21
lm233 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd233)
Anova(lm233, type = 2)
coef(summary(lm233))
#significant effect of warmingtreatment

#August 25
lm237 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd237)
Anova(lm237, type = 2)
coef(summary(lm237))
#strong significant effect of warmingtreatment **changed + marginal pred

#August 28
lm240 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd240)
Anova(lm240, type = 2)
coef(summary(lm240))
#nothing significant

#Sept 2
lm245 <- glmmTMB(focal_aphids ~ warmingtreatment * predatortreatment, family = nbinom2, data = jd245)
Anova(lm245, type = 2)
coef(summary(lm245))
...
# used binomial model with AR1
dispersal_model_binom <- glmmTMB(
  cbind(focal_winged, focal_wingless) ~ warmingtreatment * predatortreatment * jd_sc +
    warmingtreatment * predatortreatment * I(jd_sc^2) + ar1(count + 0 | cage),
  family = binomial(),
  data = countdata)
Anova(dispersal_model_binom, type = 2)
fixef(dispersal_model_binom)
...
dispersal_model_AR <- glmmTMB(dispersed ~ warmingtreatment * 
                                   predatortreatment*jd + 
                                   ar1(count + 0 | cage),
                              family = gaussian,
                              data = countdata_dispersed)
Anova(dispersal_model_AR, type = 2)
fixef(dispersal_model_AR)

#### DAY BY DAY DISPERSAL LIKELIHOOD ANALYSIS ####
...
# rescale jd and focal_aphids because R would not recognize it
...
# run glmm with AR1
sentinel_dispersed_model <- glmmTMB(cbind(sentinel_aphids, sticky_aphids) ~ 
                                    warmingtreatment * predatortreatment *
                                    jd + ar1(count + 0| cage),
                                  family = binomial, 
                                  data = countdata_dispersed)
...
#### DAY BY DAY DISPERSAL DIRECTION ANALYSIS ####
